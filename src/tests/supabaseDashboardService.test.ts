  ...
  teacher_invites: [
    {
      ...
      activated_teacher_profile_id: null,
      activated_at: null,
      created_at: '2026-05-19T08:00:00.000Z',
      updated_at: '2026-05-19T08:00:00.000Z',
    },
  ...
  student_progress_events: [
    {
      ...
      region_id: 'algebra-forge',
      activity_type: 'field_guide',
      content_id: null,
      question_id: null,
      skill_id: null,
      event_type: 'field_guide_completed',
    ...
    },
    {
      ...
      content_id: null,
      question_id: '9709_s23_qp32_q1',
      skill_id: null,
      event_type: 'practice_attempt_saved',
      event_payload: { scoreRatio: 0.76, marksEarned: 19, marksAvailable: 25, durationSeconds: 240 },
    ...
    },
    {
      ...
      actor_user_id: 'student-user-1',
      region_id: 'algebra-forge',
      activity_type: 'guardian',
      content_id: null,
      question_id: '9709_s23_qp32_q2',
      skill_id: null,
      event_type: 'guardian_attempted',
      event_payload: { scoreRatio: 0.76, marksEarned: 19, marksAvailable: 25, passed: false, durationSeconds: 300 },
      created_at: '2026-05-14T08:30:00.000Z',
    },
    {
      id: 'event-trig-field-guide',
      organization_id: 'org-1',
      class_id: 'class-alpha',
      class_membership_id: 'membership-claimed',
      student_profile_id: 'student-1',
...
describe('Supabase dashboard service', () => {
  it('maps Supabase classroom setup rows into the dashboard contract', async () => {
    ...
    expect(dashboard.studentRows[0]).toMatchObject({
      ...
      lastActivityAt: '2026-05-14T08:30:00.000Z',
    });
    ...
    expect(dashboard.exportRows[0]).toMatchObject({
      ...
      lastActivity: '2026-05-14T08:30:00.000Z',
    });
    ...
    expect(dashboard.weeklySummary.exportDownloadText).toContain('hosted activity events only');
  ...
  it('treats an empty hosted organization as data, not an authorization failure or mock fallback', async () => {
    ...
    expect(fake.tableReads).toEqual(expect.arrayContaining(['classes', 'teacher_profiles', 'teacher_invites']));
    expect(fake.tableReads).not.toContain('student_profiles');
  });
  ...
  it('allows admins to pre-authorize a teacher email before an auth user exists', async () => {
    const fake = createFakeSupabaseClient();
    const service = createSupabaseDashboardDataService({
      config: validConfig,
      createClient: vi.fn(async () => fake.client),
