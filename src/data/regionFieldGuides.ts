...
const guides: Record<string, RegionFieldGuide> = {
  ...
  'differential-shrine': {
    ...
    topic: 'First-order differential equations: forming models, separating variables, finding particular solutions, and interpreting context.',
    whatToRecognize: [
      'A rate statement involving $dy/dx$, $dh/dt$, $dT/dt$, or $dx/dt$.',
      'Proportionality wording that introduces a constant such as $k$.',
      'Separable forms where variables can be gathered on opposite sides before integrating.',
      'Initial or boundary information that determines the arbitrary constant.',
    ],
    commonExamMoves: [
      'Form the differential equation from the context before solving.',
      'Separate variables before integrating.',
      'Use initial conditions after integration, not before.',
      'Use the final model to answer the requested contextual question.',
    ],
    commonTraps: [
      'Solving when the question only asks for the model.',
    ...
    workedExamples: [
      {
        title: 'First-order model setup',
        focus: 'A tank depth $h$ rises at $4$ cm/min from inflow and falls at a rate proportional to $\\sqrt h$. Write the model.',
        setup: 'Build the derivative from increasing and decreasing contributions.',
        steps: [
          'The changing quantity is $h$ and the independent variable is $t$, so the rate is $\\frac{dh}{dt}$.',
          'Inflow contributes $+4$.',
          'The leak contributes $-k\\sqrt h$ for a positive constant $k$.',
        ],
        answer: '$$ \\frac{dh}{dt}=4-k\\sqrt h $$',
        keyMove: 'Translate the rate statement before solving.',
        check: 'The model includes a derivative, a context variable, and the proportionality constant.',
        why: 'A first-order differential equation describes how the quantity changes.',
      },
      {
      ...
      {
        title: 'Model and solve',
        focus: 'A cup at temperature $T$ cools in a room at $20$ degrees C with $\\frac{dT}{dt}=-k(T-20)$ and $T=80$ when $t=0$.',
        setup: 'Separate, integrate, and then use the initial condition.',
        steps: [
          'Separate: $$ \\frac{1}{T-20}\\,dT=-k\\,dt $$',
          'Integrate: $$ \\ln|T-20|=-kt+C $$',
          'Write $T-20=Ae^{-kt}$ and use $T(0)=80$ to get $A=60$.',
        ],
        answer: '$$ T=20+60e^{-kt} $$',
        keyMove: 'Use context data after solving the separable differential equation.',
        check: 'The answer tends toward the room temperature as time increases.',
        why: 'Modeling questions require the equation, the solution, and the contextual interpretation.',
      },
    ],
    readinessChecklist: [
      'Form a first-order differential equation from a rate statement.',
      ...
      'Interpret a solved model in context.',
    ],
  },
};
