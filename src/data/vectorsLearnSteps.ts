...
const VECTORS_LEARN_DRAFTS: VectorsLearnStepDraft[] = [
  {
    id: 'learn-vectors-2d-3d-notation',
    title: '2D and 3D vector notation',
    fieldGuideTopicId: 'vectors_notation',
    stem: 'Before line geometry, vector notation must be read reliably in both column-vector form and unit-vector form.',
    prompt: 'Which column vector matches $2\\mathbf i-3\\mathbf j+4\\mathbf k$?',
    principle: 'Principle: unit-vector coefficients are read in $\\mathbf i$, $\\mathbf j$, $\\mathbf k$ order as $x$, $y$, $z$ components.',
    explanation: 'The coefficients are $2$, $-3$, and $4$, so the column vector is $\\begin{pmatrix}2\\\\-3\\\\4\\end{pmatrix}$.',
    examTransfer: 'Exam transfer: direction ratios, line equations, scalar products, and angles all depend on preserving component order and signs.',
    primary: {
      id: 'learn-vectors-2d-3d-notation-primary',
      prompt: 'Type the column-vector components for $2\\mathbf i-3\\mathbf j+4\\mathbf k$.',
      inputType: 'numeric',
      answerType: 'expression-text',
      expectedAnswer: '(2,-3,4)',
      acceptedAnswers: ['(2,-3,4)', '2,-3,4'],
      hint: 'Read the coefficients in $i$, $j$, $k$ order.',
      methodCue: '$i$ is first, $j$ second, $k$ third.',
      firstStep: 'The components are $2$, $-3$, $4$.',
      workedRoute: [
        'The $\\mathbf i$ coefficient is $2$.',
        'The $\\mathbf j$ coefficient is $-3$.',
        'The $\\mathbf k$ coefficient is $4$.',
      ],
      skillId: 'p3_vec_3d_geometry_modelling',
    },
    similar: {
      id: 'learn-vectors-2d-3d-notation-similar',
      prompt: 'Write $\\begin{pmatrix}-1\\\\5\\end{pmatrix}$ in unit-vector form.',
      inputType: 'numeric',
      answerType: 'expression-text',
      expectedAnswer: '-i+5j',
      acceptedAnswers: ['-i+5j', '-1i+5j', '-\\mathbf i+5\\mathbf j'],
      hint: 'A 2D column uses $\\mathbf i$ and $\\mathbf j$ components.',
      methodCue: 'First component is the $i$ coefficient.',
      firstStep: '$-\\mathbf i+5\\mathbf j$.',
      workedRoute: [
        'The first component is $-1$.',
        'The second component is $5$.',
        'So the vector is $-\\mathbf i+5\\mathbf j$.',
      ],
      skillId: 'p3_vec_3d_geometry_modelling',
    },
  },
  {
    ...
    examTransfer: 'Exam transfer: line and angle questions often start by forming a direction vector from two points. Produce the component vector first, then use it in a line equation or scalar product.',
    primary: {
  ...
  {
    id: 'learn-vectors-magnitude-unit-direction-ratios',
    title: 'Magnitude, unit vectors, and direction ratios',
    fieldGuideTopicId: 'vectors_magnitude_unit_parallel',
    stem: 'A vector direction can be recorded by components, direction ratios, or a unit vector.',
    prompt: 'Find a unit vector in the direction of $(3,4,0)$.',
    principle: 'Principle: a unit vector is the original vector divided by its magnitude.',
    explanation: 'The magnitude of $(3,4,0)$ is $5$, so the unit vector is $(3/5,4/5,0)$. The direction ratios are $3:4:0$.',
    examTransfer: 'Exam transfer: line directions, angle formulae, and parallel checks all need clean magnitude and scalar-multiple work first.',
    primary: {
      id: 'learn-vectors-magnitude-unit-direction-ratios-primary',
      prompt: 'Find a unit vector in the direction of $(3,4,0)$.',
      inputType: 'numeric',
      answerType: 'coordinate',
      expectedAnswer: '(3/5,4/5,0)',
      acceptedAnswers: ['(3/5,4/5,0)', '3/5,4/5,0'],
      hint: 'First find the magnitude.',
      methodCue: '$|(3,4,0)|=5$.',
      firstStep: 'Divide every component by $5$.',
      workedRoute: [
        'The magnitude is $\\sqrt{3^2+4^2+0^2}=5$.',
        'Divide by $5$.',
        'A unit vector is $(3/5,4/5,0)$.',
      ],
      skillId: 'p3_vec_3d_geometry_modelling',
    },
    similar: {
      id: 'learn-vectors-magnitude-unit-direction-ratios-similar',
      prompt: 'Which direction ratios match vector $(6,-2,4)$?',
      inputType: 'multiple_choice',
      answerType: 'expression-text',
      expectedAnswer: '3:-1:2',
      acceptedAnswers: ['3:-1:2', 'correct'],
      ...
      hint: 'Direction ratios may be simplified by a common non-zero factor.',
      methodCue: 'Divide $6:-2:4$ by $2$.',
      firstStep: '$6:-2:4=3:-1:2$.',
      workedRoute: [
        'Use the components as ratios.',
        'Divide all three by $2$.',
        'The simplified direction ratios are $3:-1:2$.',
      ],
      skillId: 'p3_vec_3d_geometry_modelling',
    },
  },
  {
    id: 'learn-vectors-midpoint-and-proof',
    title: 'Midpoints and geometric vector proofs',
    fieldGuideTopicId: 'vectors_geometric_add_subtract',
    stem: 'Geometric vector proofs usually start with path addition, midpoint averages, or scalar multiples of displacement vectors.',
    prompt: 'Find the midpoint of $A(2,-1,3)$ and $B(8,5,1)$.',
    principle: 'Principle: the midpoint position vector is the average of the endpoint position vectors.',
    explanation: 'Average corresponding coordinates: $((2+8)/2,(-1+5)/2,(3+1)/2)=(5,2,2)$.',
    examTransfer: 'Exam transfer: before using line geometry, prove collinearity or midpoint facts with displacement vectors and scalar multiples.',
    primary: {
      id: 'learn-vectors-midpoint-and-proof-primary',
      prompt: 'Find the midpoint of $A(2,-1,3)$ and $B(8,5,1)$.',
      inputType: 'numeric',
      answerType: 'coordinate',
      expectedAnswer: '(5,2,2)',
      acceptedAnswers: ['(5,2,2)', '5,2,2'],
      hint: 'Average each coordinate.',
      methodCue: '$M=\\frac12(A+B)$.',
      firstStep: '$((2+8)/2,(-1+5)/2,(3+1)/2)$.',
      workedRoute: [
        'Average the $x$ coordinates to get $5$.',
        'Average the $y$ coordinates to get $2$.',
        'Average the $z$ coordinates to get $2$.',
      ],
      skillId: 'p3_vec_3d_geometry_modelling',
    },
    similar: {
      id: 'learn-vectors-midpoint-and-proof-similar',
      prompt: 'If $\\overrightarrow{AB}=(2,4,6)$ and $\\overrightarrow{AC}=(1,2,3)$, what geometric fact is shown?',
      inputType: 'multiple_choice',
      answerType: 'exact-text',
      expectedAnswer: 'a b c are collinear',
      acceptedAnswers: ['a b c are collinear', 'collinear', 'correct'],
      options: expressionChoices([
      ...
      expectedOptionIds: ['correct'],
      hint: 'Compare the two displacement vectors.',
      methodCue: '$\\overrightarrow{AB}=2\\overrightarrow{AC}$.',
      firstStep: 'One vector is a scalar multiple of the other.',
      workedRoute: [
        '$\\overrightarrow{AB}=(2,4,6)$.',
        '$\\overrightarrow{AC}=(1,2,3)$.',
        'Since $\\overrightarrow{AB}=2\\overrightarrow{AC}$, the points are collinear.',
      ],
      skillId: 'p3_vec_3d_geometry_modelling',
    },
  },
  {
    ...
    examTransfer: 'Exam transfer: when a question gives two points on a line, form the direction vector first, then write $\\mathbf r=\\mathbf a+\\lambda\\mathbf d$ from either point. Do not use the second point as the direction.',
    primary: {
